...
var Browser = function(id, nameOrCls, emitter) {
  ...
  this._execCommand = function(cmd) {
    ...
    this._process.on('exit', function() {
      if (!browser.isCaptured) {
        ...
        emitter.emit('browser_failure');
      }
      if (browser.isCaptured && !browser.isBeingKilled) {
        log.error('Browser process crashed');
        emitter.emit('browser_failure');
      }
...
var Launcher = function(emitter) {
  ...
  this.launch = function(names, port, urlRoot) {
    ...
    names.forEach(function(nameOrCls) {
      ...
      browser = new Cls(id, nameOrCls, emitter);
      browser.id = id;
      browser.isCaptured = false;
      browser.isBeingKilled = false;
